...
if scenario_idx == 1 %% Pedestrian moves from farside
    if x > ped_trig
...
elseif scenario_idx == 2 %% Pedestrian moves from nearside
    if x > ped_trig
...
elseif scenario_idx >= 3 % pedestrian stay at initial point
    flag_out = flag;
    ped_pos_out = ped_pos;
    ...
    %% Reward
    [reward , done_car,bump] = reward_fn( next_state,prev_state,ped_pos_out, prev_bump, local_step, max_steps);
